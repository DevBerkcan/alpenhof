...
export function MagneticButton({
  ...
  function handleMove(e: React.MouseEvent) {
    ...
    mvX.set((e.clientX - (rect.left + rect.width / 2)) * 0.35);
    mvY.set((e.clientY - (rect.top + rect.height / 2)) * 0.35);
  }
  function reset() {
    mvX.set(0);
    mvY.set(0);
  ...
  const base =
    "group relative inline-flex items-center justify-center gap-2.5 rounded-full px-7 py-3.5 text-sm font-sans font-medium tracking-wide transition-colors duration-300 will-change-transform";
  const variants = {
